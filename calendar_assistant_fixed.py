...
FOUNDER_MINDSET_PROMPT = (
    ...
    "Use casual, high-energy language (Yo founder! LFG! This is HUGE), emojis (🚀💰🔥💪), "
...
def notify_meeting_brief(brief):
    Notifier.notify(f"Yo founder! {brief} 🚀💰", title="🔥 Meeting Brief (Revenue First)")
...
def run_assistant():
    ...
    if any(kw in title for kw in low_value_keywords):
        print(f"Yo founder! Auto-declined low-value meeting: '{event.get('summary', '')}' to protect your time for revenue. 💰")
    print(f"Yo founder! Auto-declined low-value meeting: '{event.get('summary', '')}' to protect your time for revenue. 💰")
...
# Update CLI review to use founder style
    ...
    for draft in sorted(drafts, key=lambda d: -d.get('confidence_score', 0)):
        ...
        if choice == 'a':
            ...
            print("LFG! Approved. (Would send email here.) 🚀")
    ...
    print("\nReview complete. Keep the momentum! 💰🔥")
    ...
    if approval_metrics['email']['approved'] >= 50 and approval_metrics['email']['edited'] == 0:
        print("\n[TRUST] 50+ approved drafts with no edits. Suggest: Enable auto-send for routine confirmations? LFG! 🚀")
...
class ChiefOfStaffBrain:
    ...
    def analyze_everything(self, emails, calendar, docs):
        """Agentic loop: explore → analyze → reflect → improve"""
